    ...
    return (
        <div className={styles.userPanel}>
            <div style={{ display: "flex", gap: 130 }}>
                <div className={styles.panelUserStatus} style={{ borderColor: statusMap[user.status] }}>
                    <span style={{ background: statusMap[user.status] }}></span>
                    <p>{user.status}</p>
                </div>

                <button onClick={() => setShowPreview("")} style={{ border: 0, background: "transparent" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="23" height="23" fill="currentColor" className="bi bi-x-diamond-fill" viewBox="0 0 16 16">
            ...
            <div style={{display: "flex", gap: 10}}>
                ...
                <div style={{display: "flex", flexDirection: "column", gap: 3}}>
                    <span style={{fontSize: "30px"}} className={styles.userQC}>{user.displayName}</span>
            ...
            <textarea value={user.bio} className={styles.bioPanel} rows={10} cols={50} readOnly></textarea>
...
export function TabTitle({ title, setSelectedTab, index }: {title: string, setSelectedTab: Dispatch<SetStateAction<number>>, index: number}) {
    return (
        <button onClick={() => setSelectedTab(index)}>{title}</button>
    )
}

export function Tabs({ children, selectedTab }: {children: ReactNode[], selectedTab: number}) {
